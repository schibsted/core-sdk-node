...
const { isNonEmptyObj, isNonEmptyStr, isUrl, isFunction } = require('./validate');
...
class CoreApi {
     ...
     * @param {object} [options.auth] - an object that describes authentication method. It either contains
     *        clientId and clientSecret or accessToken (and optionally refreshToken).
     * @param {string} [options.auth.clientId] - the client id. If this is present, clientSecret must also
     *        be present.
     * @param {string} [options.auth.clientSecret] - client secret. If this is present, clientId must also
     *        be present.
     * @param {string} [options.auth.accessToken] - the access token. If this is present, an optional
     *        refreshToken exist too.
     *        note that if the refreshToken is provided the value of accessToken will be
     *        automatically updated in the object
     *        passed to this parameter.
     * @param {string} [options.refreshToken] - the refresh token
     * @throws if both clientId&clientSecret and accessToken&refreshToken authentication methods
     *         are present.
     */
    constructor({ serverUrl, auth, fetch }) {
    ...
    _call(method, pathname, data = {}, retry = true) {
        ...
        if (isNonEmptyObj(this.auth)) {
            opt.headers = {
                Authorization: this.auth.clientId ?
                    basicAuthHeader(this.auth.clientId, this.auth.clientSecret) :
